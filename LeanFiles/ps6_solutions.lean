...
theorem problem_1_sol {a b c : Prop} :  (a →  (b ∧  c)) →  ((a ∧ a) →  (b ∧ c)) := by
intro h1 ⟨h2,h3⟩
apply h1
exact h2
...
theorem problem_2_sol :∀ (a b c:Prop),  ((a →  c) ∧  (b → c)) →  ((a ∧ b) →  c) := by
...
theorem problem_3_sol {a d : ℝ} : ((d + a) + (a - d))  = (2 * a)  := by
calc
  ((d + a) + (a - d)) = ((d + a) + ((0 - d) + a)) := by rw [zero_sub_add_eq_sub]
  _                 = ((a + d) + ((0 - d) + a)) := by nth_rw 2 [add_comm]
...
theorem problem_4_sol {a b c d : ℝ} : (c = (d + a) + b) →  (b = a - d) → c = 2 * a  := by
  intro h1 h2
  rw [h1]
  rw [h2]
  exact problem_3_sol
...
theorem problem_5_sol {a b :ℝ } : (b*b=0) → (a+b)^2 = a^2 := by
  intro h
  have h1 : b=0 := by
    rw [<- mul_self_eq_zero]
    exact h
  ...
  _         = a*a + a*b + (b*a + 0) := by rw [h]
  _         = a*a + a*b + b*a := by rw [add_zero]
  _         = a*a + a*0 + b*a := by rw [h1]
  _         = a*a + a*0 + 0*a := by rw [h1]
  _         = a*a + 0 + 0*a := by rw [mul_zero]
  _         = a*a + 0*a := by rw [add_zero]
  _         = a*a + 0 := by rw [zero_mul]
  _         = a*a := by rw [add_zero]
  _         = a^2 := by rw [pow_two]

theorem problem_5_sol_2 {a b :ℝ } : (b*b=0) → (a+b)^2 = a^2 := by
intro h
have h1 : b=0 := by
    rw [<- mul_self_eq_zero]
    exact h
...
rw [add_zero]
rw [h1]
rw [mul_zero,add_zero,zero_mul,add_zero]
